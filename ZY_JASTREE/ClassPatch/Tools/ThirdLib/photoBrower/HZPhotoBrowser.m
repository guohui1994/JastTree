...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = [HZPhotoBrowserBackgrounColor colorWithAlphaComponent:0.4];
...
- (void)layoutSubviews
{
    ...
    _indexLabel.bounds = CGRectMake(0, 0, SJHeight + 10, 30);
    _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, SJHeight - 15);
    _saveButton.frame = CGRectMake(kDeviceWidth - 135, self.bounds.size.height - 70, 105, 30);
}
...
- (UIView *)coverView{
    if (!_coverView) {
        ...
        _coverView.backgroundColor = [HZPhotoBrowserBackgrounColor colorWithAlphaComponent:0.4];
    }
    return _coverView;
}
...
- (void)setupToolbars
{
    ...
    [saveButton setTitle:@"保存至相册" forState:UIControlStateNormal];
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if (view.beginLoadingImage) return;
     if ([_imageArray[index] isKindOfClass:[NSString class]]) {
    if ([self highQualityImageURLForIndex:index]) {
        ...
        view.imageview.image = [self placeholderImageForIndex:index];
         ...
         [view setImageWithImage:_imageArray[index][@"image"]];
     }
...
- (void)cancelPrepareForHide{
    ...
    _scrollView.hidden = NO;
    self.backgroundColor = [HZPhotoBrowserBackgrounColor colorWithAlphaComponent:0.4];
    _contentView.backgroundColor = [HZPhotoBrowserBackgrounColor colorWithAlphaComponent:0.4];
}
...
- (void)show
{
    ...
    UIWindow *window = [UIApplication sharedApplication].keyWindow;
    window.backgroundColor = [HZPhotoBrowserBackgrounColor colorWithAlphaComponent:0.4];
